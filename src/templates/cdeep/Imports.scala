...
trait BaseGenImports extends ForgeCodeGenBase {
  ...
  def emitDSLImports(stream: PrintWriter) {
    ...
    stream.println("import " + dsl.toLowerCase() + ".cdeep._")
    stream.println("import " + dsl.toLowerCase() + ".cdeep.ops._")
    if (OpsGrp.exists(t => isTpeClass(t._1))) {
      stream.println("import " + dsl.toLowerCase() + ".cdeep.typeclass._")
    }
    // stream.println("import " + dsl.toLowerCase() + ".cdeep.extern._")
  }
