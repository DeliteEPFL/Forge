...
object TablePrinterRecShallow {
  ...
  private def getFields(record: Any) = {
    try {
      record.toString.stripPrefix("Rec { ").stripSuffix(" }").split(", ").map(x => x.substring(0, x.indexOf(" = ")))
  ...
  private def readColumn(record: Any, col: String) = {
    try {
      record.getClass.getMethod("__dataAny", classOf[String]).invoke(record, col)
    } catch { case e: NoSuchMethodException => 
      ""
    }
  }
